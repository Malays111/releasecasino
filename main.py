...
async def on_startup():
    print("Vanish Casino Bot запущен!")
    print("Команды:")
...
async def on_shutdown():
    ...
    try:
        ...
        await bot.async_db._force_checkpoint()
        print("💾 Контрольная точка WAL выполнена")

        # Дополнительное принудительное сохранение
        await asyncio.sleep(0.5)  # Увеличиваем время на запись

        # Еще одно принудительное сохранение для надежности
        await bot.async_db._force_checkpoint()
        print("💾 Дополнительная контрольная точка WAL выполнена")
...
async def run_bot():
    ...
    try:
        ...
        # Принудительное сохранение при остановке пользователем
        try:
            await on_shutdown()
        except:
            pass
        ...
        # Принудительное сохранение при критической ошибке
        try:
            print("🔄 Попытка сохранить данные перед аварийным завершением...")
            await on_shutdown()
        except Exception as save_e:
            print(f"❌ Ошибка при сохранении данных: {save_e}")
